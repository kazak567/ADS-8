...
#ifndef BINARY_SEARCH_TREE_DEF
#define BINARY_SEARCH_TREE_DEF
...
private:
    struct Item {
        ...
        Item(const DataType& v) : val(v), freq(1), l_child(nullptr), r_child(nullptr) {}
    };
    ...
    int calcHeight(Item* ptr) const {
        return ptr ? 1 + std::max(calcHeight(ptr->l_child), calcHeight(ptr->r_child)) : 0;
    }

    int findCount(Item* ptr, const DataType& v) const {
        if (!ptr) return 0;
        if (ptr->val == v) return ptr->freq;
        return (v < ptr->val) ? findCount(ptr->l_child, v) : findCount(ptr->r_child, v);
    }

    void traverseInOrder(Item* ptr, std::vector<std::pair<DataType, int>>& res) const {
        if (!ptr) return;
...
public:
    ...
    std::vector<std::pair<DataType, int>> getNodesSortedByFrequency() const {
        ...
        struct Cmp {
            bool operator()(const std::pair<DataType, int>& a, const std::pair<DataType, int>& b) const {
